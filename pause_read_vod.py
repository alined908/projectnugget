...
def get_dataset(data_root_path, get_test_set=False):
    ...
    for folder in os.listdir(data_root_path):
        ...
        for image_name in os.listdir(path_to_images):
            ...
            y.append(paused_state_dic[label])
    #Convert class vector to binary class matrix
    y = keras.utils.to_categorical(y, len(paused_state_dic))
...
def pause_load_images_for_model(X_batch, resize_to_720P=True, train = False):

    X_loaded = []
    for path in X_batch:
        num_loaded = len(X_loaded)
        if num_loaded % 100 == 0:
            print("Pause: Processed " + str(num_loaded)+ " images already")
        img = cv2.imread(path)
        img = img[307:367, 550:730]
        # dividing by 255 leads to faster convergence through normalization.
        X_loaded.append(np.array(img)/(255))
    cv2.imwrite('pause_sample.jpg', X_loaded[0] * 255)
    return X_loaded
...
def train():
    ...
    X_train = np.asarray(pause_load_images_for_model(X[0:int(len(X) * 0.8)], train = True))
    y_train = np.asarray(y[0:int(len(X) * 0.8)])

    #20% of the data will be in our validation set
    X_val = np.asarray(pause_load_images_for_model(X[int(len(X) * 0.8):], train = True))
